...
# Xavier initialization of weight matrices
    ...
    for _ in range(epoch):
        for word_inputs, word_output, ngram_inputs in zip(input_seq, target_seq, char2ind):#generate n-gramlist
            ...
            activated = [i for i in stats if i != word_output][:NS]
            activated.append(word_output)
...
def main():
    ...
    # print(subsample_freqdict[100:200])
    ...
    #Training section
    emb,_ = word2vec_trainer(input_set, target_set,i2w,c2i,len(c2i),len(w2i), freqtable, NS=20, dimension=64, epoch=2, learning_rate=0.01)
    testwords = ['<department>','<knowing>','<imperfection>']
    for tw in testwords:
        sim(tw, c2i, i2c, emb)
